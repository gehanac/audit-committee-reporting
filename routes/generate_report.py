"""
routes/generate_report.py  — DAYS 6 / 8 / 11 TASKS (AI Developer 2)
---------------------------------------------------------------------
This is the most complex endpoint. It has TWO modes:

MODE 1 — Streaming (SSE):  POST /generate-report?stream=true
  • Returns tokens one-by-one as they are generated (real-time)
  • Frontend uses EventSource to display them live
  • Great for Demo Day — audience sees the AI "thinking"

MODE 2 — Async Job:  POST /generate-report  (no stream param)
  • Returns a job_id immediately (202 Accepted)
  • Report is generated in a background thread
  • Frontend polls GET /generate-report/status/<job_id>
  • Optional: supply "webhook_url" to get a callback when done

Request body (JSON):
  {
    "title":       "Q2 2026 Audit Committee Report",
    "period":      "April–June 2026",
    "items": [
      "Accounts payable approval workflow bypassed in 3 instances",
      "IT disaster recovery plan not tested in 18 months",
      "Employee expense reimbursements lack dual approval"
    ],
    "fresh":       false,         ← optional; true = skip cache
    "webhook_url": "https://..."  ← optional; called when async job finishes
  }
...
Streaming response (text/event-stream):
  ...
  data: {"token": " the"}
  ...
  data: {"done": true, "response_time_ms": 2341}

Async response (202):
  { "job_id": "abc-123-...", "status": "processing", "message": "..." }
